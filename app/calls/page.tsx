...
export default function LiveCallsPage() { 
 ...
 useEffect(() => { 
   ...
   const sub = supabaseBrowser 
     ...
     .on('postgres_changes', { event: '*', schema: 'public', table: 'transcripts', filter: `call_id=eq.${activeCall.id}` }, loadTranscripts) 
     .subscribe(); 
 
   return () => { supabaseBrowser.removeChannel(sub); }; 
 }, [activeCall]); 
 
 const currentScore = activeCall?.max_frustration_score ? Number(activeCall.max_frustration_score) : 0; 
 const scoreColor = currentScore >= 7 ? 'from-red-500 to-orange-500' : currentScore >= 5 ? 'from-yellow-500 to-orange-400' : 'from-emerald-500 to-green-500'; 
 const scoreLabel = currentScore >= 7 ? 'CRITICAL' : currentScore >= 5 ? 'ELEVATED' : 'CALM'; 
 const scoreBg = currentScore >= 7 ? 'bg-red-100 text-red-700' : currentScore >= 5 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'; 
   ...
   <main className="min-h-screen bg-slate-50"> 
     ...
     <nav className="border-b border-slate-200 sticky top-0 z-50 bg-white/95 backdrop-blur-md"> 
       <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center"> 
         ...
         <div className="hidden md:flex items-center gap-7 text-sm font-medium text-slate-700"> 
           ...
           <Link href="/calls" className="text-slate-900 font-semibold">Live Calls</Link> 
     ...
     <div className="bg-gradient-to-br from-emerald-950 via-slate-950 to-blue-950 text-white"> 
       <div className="max-w-7xl mx-auto px-6 py-14"> 
         <div className="flex items-center gap-2 text-xs uppercase tracking-widest text-emerald-300 font-semibold mb-3"> 
           <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" /> 
           Module 03 · Operations Dashboard 
         </div> 
         <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-3"> 
           What your CX team monitors all day 
         </h1> 
         <p className="text-lg text-slate-300 max-w-2xl mb-2"> 
           Every conversation Athena is having right now — voice and chat, scored turn-by-turn for frustration. Catch the angry 5% before they land on Twitter, escalate with one click, never miss a brewing fire. 
         </p> 
         <p className="text-xs text-slate-400 italic mt-4"> 
           This is the same screen Swiggy / Zomato / EatClub support leads would have open during peak hours. 
         </p> 
       </div> 
     </div> 
         ...
         <aside className="lg:sticky lg:top-24 lg:self-start lg:max-h-[calc(100vh-7rem)] lg:overflow-y-auto"> 
           <div className="rounded-2xl bg-white border border-slate-200 shadow-sm p-5"> 
             <div className="flex items-center justify-between mb-4"> 
               <div> 
                 <div className="text-xs font-semibold uppercase tracking-wider text-slate-500">Live Conversations</div> 
                 <div className="text-[10px] text-slate-400 mt-0.5">Sorted by most recent</div> 
               </div> 
             ...
             {calls.length === 0 && ( 
               <div className="p-4 rounded-xl bg-slate-50 border border-dashed border-slate-200 text-slate-500 text-sm text-center"> 
                 No calls yet. Start a chat or voice call — they appear here in real-time, just like a production support floor. 
               </div> 
             )} 
               ...
               {calls.map((c) => { 
                 ...
                 return ( 
                   ...
                   > 
                     ...
                     <div className="text-xs text-slate-500 mb-2">{new Date(c.started_at).toLocaleString()}</div> 
                     {c.max_frustration_score !== null && ( 
                       <div className="flex items-center gap-2"> 
                         <div className="flex-1 h-1.5 rounded-full bg-slate-100 overflow-hidden"> 
                           <div 
                             className={`h-full bg-gradient-to-r ${ 
                               score >= 7 ? 'from-red-500 to-orange-500' : score >= 5 ? 'from-yellow-500 to-orange-400' : 'from-emerald-500 to-green-500' 
                             }`} 
                             style={{ width: `${score * 10}%` }} 
                           /> 
                         </div> 
                         <div className="text-xs font-mono font-semibold text-slate-700 w-7 text-right">{score.toFixed(1)}</div> 
                       </div> 
                     )} 
                   </button> 
                 ); 
         ...
         <section className="space-y-6"> 
           {activeCall ? ( 
             <> 
               ...
               <div className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden"> 
                 <div className="p-6 bg-gradient-to-br from-slate-50 to-white border-b border-slate-100"> 
                   <div className="flex items-start justify-between mb-1"> 
                     ...
                     <span className={`text-[11px] px-2.5 py-1 rounded-full font-bold uppercase tracking-wider ${scoreBg}`}> 
                       {scoreLabel} 
                     </span> 
                   </div> 
                 </div> 
                 ...
                 <div className="p-8"> 
                   <div className="flex items-baseline justify-between mb-4"> 
                     <div className="text-xs font-semibold uppercase tracking-wider text-slate-500">Frustration Score</div> 
                     <div className="text-xs text-slate-500">Peak during call</div> 
                   </div> 
                   <div className="text-7xl font-bold font-mono text-slate-900 mb-4"> 
                     {currentScore.toFixed(1)}<span className="text-slate-300">/10</span> 
                   </div> 
                   ...
                   {currentScore >= 7 && ( 
                     <div className="p-3 rounded-lg bg-red-50 border border-red-200 text-sm text-red-700 flex items-center gap-2"> 
                       <span>⚠️</span> 
                       <span><strong>Auto-escalation triggered</strong> — handed off to senior CX agent with full context</span> 
                     </div> 
                   )} 
                   {currentScore < 7 && currentScore >= 5 && ( 
                     <div className="p-3 rounded-lg bg-amber-50 border border-amber-200 text-sm text-amber-700 flex items-center gap-2"> 
                       <span>⚡</span> 
                       <span>Customer frustration elevated — your CX lead should be watching this call closely</span> 
                     </div> 
                   )} 
                   {currentScore < 5 && currentScore > 0 && ( 
                     <div className="p-3 rounded-lg bg-emerald-50 border border-emerald-200 text-sm text-emerald-700 flex items-center gap-2"> 
                       <span>✓</span> 
                       <span>Resolved without human intervention — saved roughly ₹100 in agent cost</span> 
                     </div> 
                   )} 
                 </div> 
               </div> 
               ...
               <div className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden"> 
                 <div className="p-5 border-b border-slate-100 flex items-center justify-between"> 
                   <div> 
                     ...
                     <div className="text-xs text-slate-500 mt-0.5">Every turn scored for frustration in real-time</div> 
                   </div> 
                 ...
                 <div className="p-5 space-y-3 max-h-[600px] overflow-y-auto"> 
                   {transcripts.length === 0 && ( 
                     <div className="text-slate-500 text-sm text-center py-8">No transcript available for this call yet.</div> 
                   )} 
                   {transcripts.map((t) => { 
                     const isUser = t.speaker === 'user' || t.speaker === 'customer' || t.speaker === 'human'; 
                         ...
                         <div className={`max-w-[80%] ${isUser ? 'order-2' : ''}`}> 
                           ...
                           {fScore !== null && isUser && ( 
                             <div className="mt-1.5 flex items-center gap-1.5 px-1"> 
                               ...
                               <div className={`text-[10px] font-mono px-2 py-0.5 rounded-full font-semibold ${ 
                                 fScore >= 7 ? 'bg-red-100 text-red-700' : fScore >= 5 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700' 
                               }`}> 
                                 {fScore.toFixed(1)}/10 
                               </div> 
                             </div> 
               ...
               <div className="grid md:grid-cols-3 gap-4"> 
                 <div className="p-5 rounded-2xl bg-white border border-slate-200"> 
                   ...
                   <div className="font-semibold text-slate-900 text-sm mb-1">Triage in seconds</div> 
                   <p className="text-xs text-slate-500 leading-relaxed"> 
                     Sort by frustration. Hottest customers float to the top. No more reactive support. 
                   </p> 
                 </div> 
                 <div className="p-5 rounded-2xl bg-white border border-slate-200"> 
                   <div className="text-2xl mb-2">⚡</div> 
                   <div className="font-semibold text-slate-900 text-sm mb-1">Real-time alerting</div> 
                   <p className="text-xs text-slate-500 leading-relaxed"> 
                     Slack pings when frustration crosses 7. Senior agents jump in before the customer rage-quits. 
                   </p> 
                 </div> 
                 <div className="p-5 rounded-2xl bg-white border border-slate-200"> 
                   <div className="text-2xl mb-2">📞</div> 
                   <div className="font-semibold text-slate-900 text-sm mb-1">One-click takeover</div> 
                   <p className="text-xs text-slate-500 leading-relaxed"> 
                     Click any call. Read the AI handoff summary. Take over the line. Customer never knows. 
                   </p> 
                 </div> 
               </div> 
             </> 
             ...
             <div className="p-16 rounded-2xl bg-white border border-slate-200 text-center text-slate-500"> 
               Select a conversation from the left to view the live transcript and frustration trajectory. 
             </div> 
           )} 
         </section> 
       </div> 
